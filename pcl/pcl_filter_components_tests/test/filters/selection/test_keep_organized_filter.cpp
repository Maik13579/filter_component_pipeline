...
{
  auto cloud = pcl_filter_components_tests::organizedCloud();
  cloud[3].z = std::numeric_limits<float>::infinity();
  cloud.is_dense = false;

  pcl_filter_components::filters::selection::KeepOrganizedFilter<pcl_filter_components_tests::Point> filter;
