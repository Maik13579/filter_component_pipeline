// Copyright 2026 Maik Knof
// SPDX-License-Identifier: Apache-2.0

#include <gtest/gtest.h>

#include "pcl_filter_components/filters/outlier_removal/statistical_outlier_removal_filter.hpp"
#include "xyzi_filter_test_utils.hpp"

TEST(StatisticalOutlierRemovalFilterXYZI, Runs)
{
  pcl_filter_components::filters::outlier_removal::StatisticalOutlierRemovalFilter<
    pcl_filter_components_tests::Point> filter;
  filter.configure({2, 0.5, false});

  pcl_filter_components_tests::Cloud output;
  filter.filter(pcl_filter_components_tests::sampleCloud(), output);

  EXPECT_GT(output.size(), 0U);
  EXPECT_LE(output.size(), pcl_filter_components_tests::sampleCloud().size());
}
