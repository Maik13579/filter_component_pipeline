...
void expectFactoryLoadsPipeline(const std::string & pipeline_file)
{
  ...
  auto executor = std::make_shared<rclcpp::executors::MultiThreadedExecutor>(
    ...
    2,
    false);
  auto factory = std::make_shared<pcl_filter_factory::pipeline::PipelineFactoryNode>(
    executor,
    rclcpp::NodeOptions{}
    .context(context)
  ...
  EXPECT_EQ(callback_return, pcl_filter_factory::pipeline::PipelineFactoryNode::CallbackReturn::SUCCESS);
...
TEST(PipelineFactoryNode, LoadsInstalledExamplePipeline)
{
  ...
  const auto graph = pcl_filter_factory::pipeline::loadPipelineGraph(pipeline_file);
