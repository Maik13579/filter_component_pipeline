...
protected:
  void SetUp() override
  {
    ...
    char const * argv[] = {"test_pcl_filter_synchronizer"};
    context->init(1, argv);
    node = std::make_shared<rclcpp::Node>(
      "test_pcl_filter_synchronizer",
...
TEST_F(SynchronizerTest, OneInputPassThroughTriggers)
{
  ...
  pcl_filter_synchronizer::PclFilterSynchronizer sync{{}, [&ready_count]() {++ready_count;}};
...
{
  size_t ready_count = 0U;
  auto options = pcl_filter_synchronizer::SynchronizerOptions{};
  options.policy = pcl_filter_synchronizer::SyncPolicy::ApproximateTime;
  options.slop = 0.02;
  pcl_filter_synchronizer::PclFilterSynchronizer sync{options, [&ready_count]() {++ready_count;}};
  const auto qos = rclcpp::QoS{5};
...
{
  pcl_filter_synchronizer::PclFilterSynchronizer sync;
  const auto qos = rclcpp::QoS{1};
